...
typedef struct _PSMUX_UNIT_
{
    ...
    unsigned int    is_key_frame;       /* 是否关键数据(I 帧)                  */
...
/******************************************************************************
 * 功 能：生成 crc32 校验码，参考 ffmpeg
 * 参 数：buffer - 生成 crc 码的缓冲区
 *        length - 生成 crc 码的缓冲区长度
 * 返回值：crc 校验码
 ******************************************************************************/
...
/******************************************************************************
 * 功 能：根据 PES_PACK_INFO *pes 内的信息，在 buffer 里生成 PES 头数据
 * 参 数：  buffer      - PES 头缓冲区
 *           pes         - 当前处理的pes
 *           prc         - 当前处理的单元
 *           pes         - 输入输出参数结构
...
static void PSMUX_create_pes_header(PSMUX_PROCESS_PARAM *param, PES_PACK_INFO *pes)
{
    ...
    {
        /* pes_header_stuff_len 至少为 2，且要保证 pes 按 4 byte 对齐 */
...
/******************************************************************************
 * 功 能：根据 byte_rate 和 scr 信息，在 buffer 里生成 PS 头数据
 * 参 数：  buffer      - PS 头缓冲区
 *           byte_rate   - PS 流码率
 *           scr         - 系统参考时间
 *           frame_num   - 帧号
...
/******************************************************************************
 * 功 能：根据 PROGRAM_INFO *prg 信息，在 buffer 里生成 PSM 数据
 * 参 数：  buffer      - PSM 缓冲区
 *           prg         - 节目流参考信息
 *           prc         - 处理参数，主要用到全局时间
 * 返回值：PSM 数据的长度
    ...
    while (!g_stop_request)
    {
        ...
        for (i=0; !g_stop_request && i<VLC_CHANNEL_MAX; )
        {
            ...
            if (pkt)
            {
				...
				if(ps_file != NULL)
				{
					fwrite(pkt->buf, sizeof(char), pkt->len, ps_file);
				}
...
int libpes_send_to_vlc(int channel, char *ip, int port)
{
    ...
    info->memory_limit = channel == 0 ? 0x100000/TS_UNITS_MAX_SIZE : 0x80000/TS_UNITS_MAX_SIZE;
