...
int dmc_input(int media_chn, int media_type, int media_subtype, unsigned long long frame_pts,
              unsigned char *frame_data, int frame_len, int frame_end_flag)
{
    int i;
    ...
    for (i = 0; i < _DMC_SUBSCRIBER_MAX; i++) {
        ...
        if (subscribe->media_type & media_type) {
            subscribe->proc(media_chn, media_type, media_subtype, frame_pts, frame_data, frame_len,
                            frame_end_flag);
        }
    }
